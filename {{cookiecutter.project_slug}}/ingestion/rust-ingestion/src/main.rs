    ...
    if config.redis.enabled {
        if let Some(storage) = state.storage {
            match storage.get_connection().await {
                Ok(mut conn) => {
                    println!(
                        "📊 Fetching historical data from: {:?}",
                        config.data_source.source_type
                    );
                    ...
                    match config.data_source.source_type.as_str() {
                        "binance" => {
                            ...
                            for data in historical_data {
                                ...
                                if current_batch.len() >= BATCH_SIZE {
                                    storage
                                        .set_batch_ohlc_records(
                                            &mut conn,
                                            &config.provider.coin,
                                            &current_batch,
                                        )
                                        .await?;
                                    total_processed += current_batch.len();
                                    println!(
                                        "✅ Processed batch of {} items for {} (total: {})",
                                        BATCH_SIZE, config.provider.coin, total_processed
                                    );
                                    current_batch.clear();
                                }
                            }
                        ...
                        "local_csv" | "s3_csv" => {
                            ...
                            let mut batches_by_symbol: HashMap<String, Vec<OhlcRecord>> =
                                HashMap::new();
                            ...
                            for (symbol, records) in batches_by_symbol {
                                ...
                                for chunk in records.chunks(BATCH_SIZE) {
                                    storage
                                        .set_batch_ohlc_records(&mut conn, &symbol, chunk)
                                        .await?;
